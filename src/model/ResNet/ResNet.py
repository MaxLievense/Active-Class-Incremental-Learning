...
class ResNet(torch.nn.Module, Base):
    def __init__(self, n_classes: int, device: torch.device, **cfg):
        ...
        if self.cfg.pretrained and self.weights == None:
    ...
    def get_weights(self):
        if self.cfg.pretrained == "DEFAULT":
            ...
            if not self.model.__name__ in self.cfg.pretrained:
                self.log.error(
                    f"Pretrained weights arch might not match model arch: {self.model.__name__} and {self.cfg.pretrained}"
                )
                try:
                    ...
                    self.log.error(f"Could not determine model architecture from pretrained url.")
            self.weights = None
