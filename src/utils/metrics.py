...
class Metrics:
    ...
    def wandb_out(self, epoch, open=False):
        data = {
...
def tailed_metrics(
    ...
    for name, thres in [MANY_THRES, MID_THRES, FEW_THRES, XLT_THRES]:
        ...
        thres_classes = [k for k, v in mapped_training_class_counts.items() if v > thres[1] and v <= thres[0]]
...
def class_coverage(targets: torch.Tensor, n_classes, thresholds: list = [5]) -> None:
